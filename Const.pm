...
package Const::_Scalar ; # Do not use directly!
...
#    croak "Usage: tie [my] \$SCALAR, 'Const::Scalar', <scalar>"
#        if scalar @_ < 2 ;
    my $pkg  = shift ;
    my $self = shift ;
...
package Const::_Hash ; # Do not use directly!
...
#    croak "Usage: tie [my] \%HASH, 'Const::Hash', <hash>"
#        if scalar @_ < 2 ;
#    croak "Attempt to assign odd number of elements to const hash"
#        if scalar @_ % 2 == 0 ;
    my $pkg  = shift ;
...
Const - Perl module to provide constant scalars and hashes.
...
=head1 SYNOPSIS
    ...
    use Const qw( const reconst unconst ) ; # Or "use Const ':ALL' ;"
...
=head2 Constant Scalars
    ...
    use Const ':ALL' ; # or "use Const qw( const reconst unconst ) ;"
...
them back into ordinary variables again with C<unconst>.)
    ...
    use Const qw( const reconst unconst ) ;
